...
fit <- kmeans(mydata, 20, iter.max=30) #20 cluster solution
...
exclude <- c("ActiveSubs.how.many.of.contracts.active.", "notNewCellphoneUser", "AdjustmentsToCreditRating","ReferralsMadeBySubscriber","nonUSTravel","OwnsComputer", "RetentionCalls","RetentionOffersAccepted","NewCellphoneUser","PercChangeRevenues","ChildrenInHH", "HandsetRefurbished", "HandsetWebCapable", "OptOutMailings")
mydata <- mydata[ , !(names(mydata) %in% exclude)]

# fit the K-means model to mydata
fit <- kmeans(mydata, 30) #30 cluster solution
...
fit <- kmeans(mydata, 30, iter.max=30) #30 cluster solution
